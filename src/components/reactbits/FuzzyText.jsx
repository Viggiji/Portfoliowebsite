  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const run = timestamp => {
        ...
        } else { currentIntensity = targetIntensity; }
      ...
      const handleMouseMove = e => { if (!enableHover) return; const rect = canvas.getBoundingClientRect(); isHovering = isInsideTextArea(e.clientX - rect.left, e.clientY - rect.top); };
      ...
      const handleTouchMove = e => { if (!enableHover) return; e.preventDefault(); const rect = canvas.getBoundingClientRect(); const touch = e.touches[0]; isHovering = isInsideTextArea(touch.clientX - rect.left, touch.clientY - rect.top); };
